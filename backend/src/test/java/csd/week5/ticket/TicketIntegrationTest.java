...
// 		URI uri = new URI(baseUrl + port + "/tickets");
// 		Ticket Ticket = new Ticket("BTS Concert", "123456789", "1k65cv", "100");
// 		users.save(new User("admin", encoder.encode("goodpassword"), "ROLE_ADMIN", "admin@gmail.com", "SMU building", "12345678"));
...
// 	@Test
// 	public void buyTicket_ValidTicketId_Success() throws Exception {
// 		Ticket Ticket = tickets.save(new Ticket("BTS Concert", "123456789", "1k65cv", "100"));
// 		User user = users.save(new User("testUser", encoder.encode("goodpassword"), "ROLE_USER", "user@gmail.com", "SMU building", "12345678"));
// 		URI uri = new URI(baseUrl + port + "/tickets/" + Ticket.getId().longValue() + "/" + user.getId().longValue() + "/buy");
		
// 		ResponseEntity<Ticket> result = restTemplate.withBasicAuth("testUser", "goodpassword")
// 										.exchange(uri, HttpMethod.PUT, null, Ticket.class);
// 		assertEquals(200, result.getStatusCode().value());
// 		assertEquals(false, result.getBody().getAvailable());
// 	}
// }
